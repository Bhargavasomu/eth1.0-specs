...
def to_valid_address(data: bytes) -> Address:
    """
    Convert a byte stream to a valid address (20 bytes).
    ...
    -------
    byte_stream : `bytes`
        The byte stream obtained from the passed string.
    """
    if len(data) == 20:
        return data

    if len(data) < 20:
        return data.rjust(20, b"\x00")
